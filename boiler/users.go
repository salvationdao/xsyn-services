...
func (userL) LoadCreditTransactions(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.Credit) {
...
func (userL) LoadDebitTransactions(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.Debit) {
...
func (o *User) AddCreditTransactions(exec boil.Executor, insert bool, related ...*Transaction) error {
	...
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.Credit, o.ID)
			if err = rel.Insert(exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
...
// SetCreditTransactions removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.CreditUser's CreditTransactions accordingly.
// Replaces o.R.CreditTransactions with related.
// Sets related.R.CreditUser's CreditTransactions accordingly.
func (o *User) SetCreditTransactions(exec boil.Executor, insert bool, related ...*Transaction) error {
	query := "update \"transactions\" set \"credit\" = null where \"credit\" = $1"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	if o.R != nil {
		for _, rel := range o.R.CreditTransactions {
			queries.SetScanner(&rel.Credit, nil)
			if rel.R == nil {
				continue
			}

			rel.R.CreditUser = nil
		}

		o.R.CreditTransactions = nil
	}
	return o.AddCreditTransactions(exec, insert, related...)
}

// RemoveCreditTransactions relationships from objects passed in.
// Removes related items from R.CreditTransactions (uses pointer comparison, removal does not keep order)
// Sets related.R.CreditUser.
func (o *User) RemoveCreditTransactions(exec boil.Executor, related ...*Transaction) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(exec, boil.Whitelist("credit")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.CreditTransactions {
			if rel != ri {
				continue
			}

			ln := len(o.R.CreditTransactions)
			if ln > 1 && i < ln-1 {
				o.R.CreditTransactions[i] = o.R.CreditTransactions[ln-1]
			}
			o.R.CreditTransactions = o.R.CreditTransactions[:ln-1]
			break
		}
	}
...
// SetDebitTransactions removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.DebitUser's DebitTransactions accordingly.
// Replaces o.R.DebitTransactions with related.
// Sets related.R.DebitUser's DebitTransactions accordingly.
func (o *User) SetDebitTransactions(exec boil.Executor, insert bool, related ...*Transaction) error {
	query := "update \"transactions\" set \"debit\" = null where \"debit\" = $1"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	if o.R != nil {
		for _, rel := range o.R.DebitTransactions {
			queries.SetScanner(&rel.Debit, nil)
			if rel.R == nil {
				continue
			}
		...
		o.R.DebitTransactions = nil
	}
	return o.AddDebitTransactions(exec, insert, related...)
}

// RemoveDebitTransactions relationships from objects passed in.
// Removes related items from R.DebitTransactions (uses pointer comparison, removal does not keep order)
// Sets related.R.DebitUser.
func (o *User) RemoveDebitTransactions(exec boil.Executor, related ...*Transaction) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(exec, boil.Whitelist("debit")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.DebitTransactions {
			if rel != ri {
				continue
			}

			ln := len(o.R.DebitTransactions)
			if ln > 1 && i < ln-1 {
				o.R.DebitTransactions[i] = o.R.DebitTransactions[ln-1]
			}
			o.R.DebitTransactions = o.R.DebitTransactions[:ln-1]
			break
		}
	}
