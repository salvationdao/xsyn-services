...
func NewOrganisationController(log *zerolog.Logger, conn *pgxpool.Pool, api *API) *OrganisationController {
	...
	api.SecureCommandWithPerm(HubKeyOrganisationList, organisationHub.ListHandler, types.PermOrganisationList)
	api.SecureCommandWithPerm(HubKeyOrganisationGet, organisationHub.GetHandler, types.PermOrganisationRead)
	api.SecureCommandWithPerm(HubKeyOrganisationCreate, organisationHub.CreateHandler, types.PermOrganisationCreate)
	api.SecureCommandWithPerm(HubKeyOrganisationUpdate, organisationHub.UpdateHandler, types.PermOrganisationUpdate)
	api.SecureCommandWithPerm(HubKeyOrganisationArchive, organisationHub.ArchiveHandler, types.PermOrganisationArchive)
	api.SecureCommandWithPerm(HubKeyOrganisationUnarchive, organisationHub.UnarchiveHandler, types.PermOrganisationUnarchive)
...
type OrganisationListResponse struct {
	Records []*types.Organisation `json:"records"`
...
type OrganisationGetRequest struct {
	*hub.HubCommandRequest
	Payload struct {
		...
		ID   types.OrganisationID `json:"id"`
	} `json:"payload"`
}
...
func (ctrlr *OrganisationController) UpdateHandler(ctx context.Context, hubc *hub.Client, payload []byte, reply hub.ReplyFunc) error {
	...
	var oldOrganisation types.Organisation = *organisation
