...
func FactionCreate(ctx context.Context, conn Conn, faction *types.Faction) error {
	q := `
		INSERT INTO 
			factions (
				id, 
...
func FactionGet(ctx context.Context, conn Conn, factionID types.FactionID) (*types.Faction, error) {
	result := &types.Faction{}
...
func FactionAll(ctx context.Context, conn Conn) ([]*types.Faction, error) {
	result := []*types.Faction{}
...
// FactionGetByUserID
func FactionGetByUserID(ctx context.Context, conn Conn, userID types.UserID) (*types.Faction, error) {
	result := &types.Faction{}
	q := `
		SELECT
			f.*
...
func FactionGetRecruitNumber(ctx context.Context, conn Conn, factionID types.FactionID) (int64, error) {
...
func FactionMvpGet(ctx context.Context, conn Conn, factionID types.FactionID) (*types.UserBrief, error) {
...
func FactionSupsVotedGet(ctx context.Context, conn Conn, factionID types.FactionID) (*types.BigInt, error) {
	var wrap struct {
		Sups types.BigInt `db:"sups_voted"`
	}
	q := `
