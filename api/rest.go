...
func WithUser(api *API, next func(w http.ResponseWriter, r *http.Request, user *passport.User) (int, error)) func(w http.ResponseWriter, r *http.Request) (int, error) {
...
func GetUserFromToken(api *API, r *http.Request) (*passport.User, int, error) {
	...
	token, err := db.AuthFindToken(ctx, api.Conn, passport.IssueTokenID(jwtID))
	if err != nil {
		return nil, http.StatusUnauthorized, terror.Error(err)
	}
