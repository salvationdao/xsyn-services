...
SET attributes = JSONB_SET(attributes, item.path, '1530', FALSE)
    FROM item
WHERE xsyn_store.attributes @> '[{"trait_type": "Rarity", "value": "Colossal"}]' -- rarity we want to update
...
SET attributes = JSONB_SET(attributes, item.path, '1560', FALSE)
    FROM item
WHERE xsyn_store.attributes @> '[{"trait_type": "Rarity", "value": "Rare"}]' -- rarity we want to update
  AND faction_id = (SELECT id FROM factions where factions.label = 'Red Mountain Offworld Mining Corporation'); -- faction we want to update
...
SET attributes = JSONB_SET(attributes, item.path, '1590', FALSE)
    FROM item
WHERE xsyn_store.attributes @> '[{"trait_type": "Rarity", "value": "Legendary"}]' -- rarity we want to update
  AND faction_id = (SELECT id FROM factions where factions.label = 'Red Mountain Offworld Mining Corporation'); -- faction we want to update
...
SET attributes = JSONB_SET(attributes, item.path, '1620', FALSE)
    FROM item
WHERE xsyn_store.attributes @> '[{"trait_type": "Rarity", "value": "Elite Legendary"}]' -- rarity we want to update
  AND faction_id = (SELECT id FROM factions where factions.label = 'Red Mountain Offworld Mining Corporation'); -- faction we want to update
...
SET attributes = JSONB_SET(attributes, item.path, '1650', FALSE)
    FROM item
WHERE xsyn_store.attributes @> '[{"trait_type": "Rarity", "value": "Ultra Rare"}]' -- rarity we want to update
  AND faction_id = (SELECT id FROM factions where factions.label = 'Red Mountain Offworld Mining Corporation'); -- faction we want to update
...
SET attributes = JSONB_SET(attributes, item.path, '1650', FALSE)
    FROM item
WHERE xsyn_store.attributes @> '[{"trait_type": "Rarity", "value": "Exotic"}]' -- rarity we want to update
  AND faction_id = (SELECT id FROM factions where factions.label = 'Red Mountain Offworld Mining Corporation'); -- faction we want to update
...
SET attributes = JSONB_SET(attributes, item.path, '1710', FALSE)
    FROM item
WHERE xsyn_store.attributes @> '[{"trait_type": "Rarity", "value": "Guardian"}]' -- rarity we want to update
  AND faction_id = (SELECT id FROM factions where factions.label = 'Red Mountain Offworld Mining Corporation'); -- faction we want to update
...
SET attributes = JSONB_SET(attributes, item.path, '1740', FALSE)
    FROM item
WHERE xsyn_store.attributes @> '[{"trait_type": "Rarity", "value": "Mythic"}]' -- rarity we want to update
  AND faction_id = (SELECT id FROM factions where factions.label = 'Red Mountain Offworld Mining Corporation'); -- faction we want to update
...
SET attributes = JSONB_SET(attributes, item.path, '1800', FALSE)
    FROM item
WHERE xsyn_store.attributes @> '[{"trait_type": "Rarity", "value": "Deus ex"}]' -- rarity we want to update
  AND faction_id = (SELECT id FROM factions where factions.label = 'Red Mountain Offworld Mining Corporation'); -- faction we want to update
