...
func BlobGet(ctx context.Context, conn Conn, blobID passport.BlobID) (*passport.Blob, error) {
	blob := &passport.Blob{}
	q := `
...
func BlobGetByFilename(ctx context.Context, conn Conn, fileName string) (*passport.Blob, error) {
	blob := &passport.Blob{}
	q := `
		SELECT id, file_name, file_size_bytes, extension, mime_type, file, views, public
		FROM blobs
...
func BlobGetByHash(ctx context.Context, conn Conn, hash string, fileSizeBytes int64) (*passport.Blob, error) {
	blob := &passport.Blob{}
	q := `
		SELECT id, file_name, file_size_bytes, extension, mime_type, file, views, public
		FROM blobs
...
func BlobInsert(ctx context.Context, conn Conn, blob *passport.Blob) error {
	q := `--sql
...
func BlobUpdate(ctx context.Context, conn Conn, blob *passport.Blob) error {
	q := `--sql
		UPDATE blobs 
...
func BlobList(ctx context.Context, conn Conn, search string, offset int, limit int) (int, *[]*passport.Blob, error) {
	args := []interface{}{}
	var totalRows int
	result := []*passport.Blob{}
