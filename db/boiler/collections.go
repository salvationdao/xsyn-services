...
var (
	...
	collectionColumnsWithoutDefault = []string{"name", "slug"}
	collectionColumnsWithDefault    = []string{"id", "logo_blob_id", "keywords", "deleted_at", "updated_at", "created_at", "mint_contract", "stake_contract"}
...
func (o *Collection) XsynAssets(mods ...qm.QueryMod) xsynAssetQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"xsyn_assets\".\"collection_id\"=?", o.ID),
	)
...
func (o *Collection) XsynStores(mods ...qm.QueryMod) xsynStoreQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"xsyn_store\".\"collection_id\"=?", o.ID),
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (collectionL) LoadXsynAssets(e boil.Executor, singular bool, maybeCollection interface{}, mods queries.Applicator) error {
	var slice []*Collection
	var object *Collection
	...
	query := NewQuery(
		qm.From(`xsyn_assets`),
		qm.WhereIn(`xsyn_assets.collection_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (collectionL) LoadXsynStores(e boil.Executor, singular bool, maybeCollection interface{}, mods queries.Applicator) error {
	var slice []*Collection
	var object *Collection
	...
	query := NewQuery(
		qm.From(`xsyn_store`),
		qm.WhereIn(`xsyn_store.collection_id in ?`, args...),
		qmhelper.WhereIsNull(`xsyn_store.deleted_at`),
	)
	if mods != nil {
		mods.Apply(query)
...
// Sets related.R.Collection appropriately.
func (o *Collection) AddXsynAssets(exec boil.Executor, insert bool, related ...*XsynAsset) error {
	var err error
	for _, rel := range related {
		if insert {
...
// SetXsynAssets removes all previously related items of the
// collection replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Collection's XsynAssets accordingly.
// Replaces o.R.XsynAssets with related.
// Sets related.R.Collection's XsynAssets accordingly.
func (o *Collection) SetXsynAssets(exec boil.Executor, insert bool, related ...*XsynAsset) error {
	query := "update \"xsyn_assets\" set \"collection_id\" = null where \"collection_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.XsynAssets {
	...
	return o.AddXsynAssets(exec, insert, related...)
}

// RemoveXsynAssets relationships from objects passed in.
// Removes related items from R.XsynAssets (uses pointer comparison, removal does not keep order)
// Sets related.R.Collection.
func (o *Collection) RemoveXsynAssets(exec boil.Executor, related ...*XsynAsset) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.XsynAssets {
			if rel != ri {
				continue
			}

			ln := len(o.R.XsynAssets)
			if ln > 1 && i < ln-1 {
				o.R.XsynAssets[i] = o.R.XsynAssets[ln-1]
			}
			o.R.XsynAssets = o.R.XsynAssets[:ln-1]
			break
		}
	}
...
// Sets related.R.Collection appropriately.
func (o *Collection) AddXsynStores(exec boil.Executor, insert bool, related ...*XsynStore) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.CollectionID, o.ID)
...
// collection replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Collection's XsynStores accordingly.
// Replaces o.R.XsynStores with related.
// Sets related.R.Collection's XsynStores accordingly.
func (o *Collection) SetXsynStores(exec boil.Executor, insert bool, related ...*XsynStore) error {
	query := "update \"xsyn_store\" set \"collection_id\" = null where \"collection_id\" = $1"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
...
// Sets related.R.Collection.
func (o *Collection) RemoveXsynStores(exec boil.Executor, related ...*XsynStore) error {
	if len(related) == 0 {
		return nil
	}
